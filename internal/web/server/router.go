...
func (s *Server) registerRoutes() {
...

	{
		...
		// Anonymous gist display route — no user required
		sAnon := r.SubGroup("/anonymous")
		{
			sAnon.Use(makeCheckRequireLogin(true), anonGistInit)
			sAnon.GET("/:gistname", gist.GistIndex)
			sAnon.GET("/:gistname/rev/:revision", gist.GistIndex)
			sAnon.GET("/:gistname/raw/:revision/:file", gist.RawFile)
			sAnon.GET("/:gistname/download/:revision/:file", gist.DownloadFile)
			sAnon.GET("/:gistname/revisions", gist.Revisions)
		}

		// Anonymous gist management by edit token
		sD := r.SubGroup("/anon")
		{
			sD.GET("/confirm/:token", gist.AnonConfirm)
			sD.GET("/edit/:token", gist.AnonEdit)
			sD.POST("/edit/:token", gist.AnonProcessEdit)
			sD.GET("/delete/:token", gist.AnonDelete)
			sD.POST("/delete/:token", gist.AnonProcessDelete)
		}
	}
