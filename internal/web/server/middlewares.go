...
func gistInit(next Handler) Handler {
	return func(ctx *context.Context) error {
		...
		if gist.Private == db.PrivateVisibility {
			// Anonymous gists cannot be private — if one slipped through, treat as not found
			if gist.IsAnonymous() {
				return ctx.NotFound("Gist not found")
			}
			if currUser == nil || gist.UserID == nil || currUser.ID != *gist.UserID {
				// Check for token-based auth via Authorization header
				if tokenUser := getUserByToken(ctx); tokenUser != nil && gist.UserID != nil && tokenUser.ID == *gist.UserID {
...
// anonGistInit loads an anonymous gist by UUID (no user JOIN required).
// Used for the /anonymous/:gistname display route.
func anonGistInit(next Handler) Handler {
	return func(ctx *context.Context) error {
		gistName := ctx.Param("gistname")
