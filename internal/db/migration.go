...
// Add EditToken for anonymous gists and make UserID nullable
func v4_anonymousGists() error {
	// Add edit_token column to gists
	if err := db.Exec("ALTER TABLE gists ADD COLUMN IF NOT EXISTS edit_token TEXT DEFAULT ''").Error; err != nil {
		// SQLite doesn't support IF NOT EXISTS on ALTER TABLE — try without
		db.Exec("ALTER TABLE gists ADD COLUMN edit_token TEXT DEFAULT ''")
	}
	// Index on edit_token for fast lookups
	return db.Exec("CREATE INDEX IF NOT EXISTS idx_gists_edit_token ON gists(edit_token)").Error
}
