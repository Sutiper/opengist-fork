...
type Gist struct {
	...
	UserID          *uint      // nullable: NULL for anonymous gists
	User            User
	NbFiles         int
	NbLikes         int
	NbForks         int
	...
	// Anonymous gist support: single token for both edit and delete
	EditToken string `gorm:"index"` // empty for authenticated gists
...
func (gist *Gist) CanWrite(user *User) bool {
	...
	return user != nil && gist.UserID != nil && *gist.UserID == user.ID
}

func (gist *Gist) CanWriteWithToken(user *User, token string) bool {
	if gist.IsAnonymous() {
		return token != "" && gist.EditToken == token
	}
	return gist.CanWrite(user)
}

func GetGistByEditToken(token string) (*Gist, error) {
	gist := new(Gist)
	err := db.Preload("User").Where("edit_token = ?", token).First(gist).Error
	return gist, err
}

// DeleteExpiredAnonymousGists removes anonymous gists that have not been updated
// for more than ttlMinutes. Called by the background scheduler.
func DeleteExpiredAnonymousGists(ttlMinutes int) error {
	if ttlMinutes <= 0 {
		return nil
	}
	...
	for _, g := range gists {
		if err := g.Delete(); err != nil {
			log.Error().Err(err).Msgf("Failed to delete expired anonymous gist %s", g.Uuid)
		}
	}
	return nil
}
...
func (gist *Gist) ToIndexedGist() (*index.Gist, error) {
	...
	// For anonymous gists, UserID is nil — use 0 in the index
	var indexUserID uint
	if gist.UserID != nil {
		indexUserID = *gist.UserID
	}
